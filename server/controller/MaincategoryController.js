...
async function createRecord(req, res) {
    try {
        ...
        if (error.keyValue)
            res.send({ status: 400, result: "Fail", message: "Maincategory Name is already Exist" })
...
async function updateRecord(req, res) {
    try {
        ...
        if (data) {
            ...
            res.send({ status: 400, result: "Fail", message: "Record Not Found" })
        }
    } catch (error) {
        if (error.keyValue)
            res.send({ status: 404, result: "Fail", message: "Maincategory Name is Already Exist" })
        else
            res.send({ status: 500, result: "Fail", message: "Internal Server Error" })
    }
}
async function deleteRecord(req, res) {
    try {
        ...
        if (data) {
            ...
            res.sen({ status: 404, result: "Fail", message: "Record Not Found" })
        }
    } catch (error) {
        res.send({ status: 500, result: "Fail", message: "Internal Server Error" })
    }
