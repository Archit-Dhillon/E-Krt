...
async function getSingleRecord(req, res) {
    try {
        ...
        if (data)
            ...
            res.send({ status: 404, result: "Fail", message: "Data not Found" })
...
async function updateRecord(req, res) {
    try {
        ...
        if (data) {
            ...
            await data.save()
            res.send({ status: 200, result: "Done", message: "Record Update" });
        }

    } catch (error) {
        if (error.keyValue)
            res.send({ status: 400, result: "Fail", message: "Product Already Exist" })
        else
            res.send({ status: 500, result: "Fail", message: "Internal Server Error" })
    }
}
...
async function search(req, res) {
    ...
    try {
        let data = await Product.find({
            $or: [{ name: { $regex: search, Option: "i" } },
            { maincategory: search }, { maincategory: search },
            { subcategory: search },
            { brand: search },
            { color: search },
            { size: search },
        ...
        res.send({ status: 200, result: "Done", daat: data, count: data.length })
