...
export default function Brand() {
  const columns = [
    ...
    },
    {
      ...
      sortable: false,
      renderCell: ({ row }) => <Link
        to={`/admin/brand/updatebrand/${row._id}`}
      >
  ...
  useEffect(() => {
    getAPIData();
  }, [BrandStateData.length]); //huma useEffect ma aab dependency lagani padagi kyo ke iss sa dependency diya bina ya sirf ek bar call hoga islya dependency laga danga jitne bar state change ho ya unte bar update ho jaay
        ...
        <div className="row">
          ...
          <div className="col-lg-9 col-md-9 ">
            <h5 className="text-dark  rad text-center mt-2  p-2">
              Brand
              <Link to="/admin/brand/createbrand">
                  ...
                  {data.map((item, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>{item.name}</td>
                        <td>
                          <Link to={`/admin/brand/updatebrand/${item.id}`}>
                            <i className="fa fa-edit text-success"></i>
                          </Link>
                        </td>
                        <td>
