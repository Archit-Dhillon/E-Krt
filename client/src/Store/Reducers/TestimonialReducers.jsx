...
export default function TestimonialReducer(state = [], action) {
  ...
  switch (action.type) {
    ...
    case UPDATE_TESTIMONIAL_RED:
      ...
      newState[index].message = action.payload.message; // iss lya nahi kyo ke hum na stste change nehi ke balki state ke value change ke h)
      newState[index].pic = action.payload.pic;
      newState[index].star = action.payload.star;
      newState[index].profession = action.payload.profession;
